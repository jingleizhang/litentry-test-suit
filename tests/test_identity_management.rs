...
use litentry_api_client::{
    ...
    utils::{
        ...
        //hex::hex_encode,
        identity::build_msg_web2,
        identity::build_vdata_discord,
...
fn tc_create_identity_discord() {
    ...
    let discord_name = ParameterString::try_from("eric#4151".as_bytes().to_vec()).unwrap();
    let identity = Identity::Web2 {
        network: Web2Network::Discord,
        address: discord_name,
    };
    let ciphertext_metadata: Option<Vec<u8>> = None;
    ...
    println!("post msg to discord: {}", msg);

    print_passed();
}

#[test]
fn tc_verify_identity_discord() {
...
fn tc_create_identity_twitter() {
    ...
    let twitter_name = ParameterString::try_from("ericzhangeth".as_bytes().to_vec()).unwrap();
    let identity = Identity::Web2 {
        network: Web2Network::Twitter,
        address: twitter_name,
    };
    let ciphertext_metadata: Option<Vec<u8>> = None;
